...
def test_build_all_services(docker_client: DockerClient) -> None:
    ...
    images = docker_client.image.list()
    ...
    print("Available images:", full_image_names)
...
def test_api_health(docker_client: DockerClient) -> None:
    ...
    try:
        ...
        for i in range(max_retries):
            try:
                print(
                    f"Attempt {i + 1}/{max_retries} to connect to API health endpoint..."
                )
                ...
                print(
                    f"API not ready yet ({e.__class__.__name__}), waiting {retry_interval}s..."
                )
